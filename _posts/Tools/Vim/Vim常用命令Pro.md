...
> 注意这种方式，如果关闭终端，挂起的 Vim 也会关闭

在 Vim 里按 `Ctrl + Z`，会让 Vim 在后台挂起并回到终端；

在终端里使用 `jobs` 命令查看挂起的 Vim；

在终端使用 `fg` 命令回到上一个挂起的 Vim；

也可以用 `%` 代替 `fg` 命令达到相同效果;

可以用 `%<序号>` 回到 `jobs` 命令显示的某个 Vim 里（例如：%1 回到第一个挂起的 Vim）。

**方式二:**

在终端内使用 `:!sh` 命令回到 bash 终端，`!` 会启动 Vim 外的一个进程，在终端输入 `exit` 可回到 Vim。

> `:!sh` 里 sh 指的是默认 shell, 也可以指定 shell，比如默认是 bash 情况下打开 zsh，则命令为：`:!zsh`。

</br>
如果你仅仅返回终端查看某些信息，则可以使用命令 `:!sh&` 回到终端，然后按 `Enter` 回到 Vim。

## 其他
- Ctrl-g: 显示当前位置的行与总行数，以及当前所在行数占总行数的比例。
- Ctrl-r: 反撤销，用于还原u的操作。
- :set scrolloff=n 设置 Vim 底部始终显示 `n` 行
